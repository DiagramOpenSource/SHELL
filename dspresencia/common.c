...
{ 
   ...
   while (*p) { 
      if (p > buf) { 
         for( p2=p-2; isdigit((unsigned char)*p2); p2--) ; 
      } 
...
{ 
   ...
   for (i=0; i<numTasks; i++) { 
      if (!tlist[i].WindowTitle[0] && tlist[i].dwProcessId == pid) { 
         tlist[i].hwnd = hwnd; 
         // 
...
int KillOthers(char *amatar)
{
   ...
   yo = GetCurrentProcessId();
   for (i = 0; i < numTasks;i++)
   {
	  if (tlista[i].dwProcessId == yo)
		  continue;
      strlwr(tlista[i].ProcessName);
      if (strstr(tlista[i].ProcessName,tmp))
      {
