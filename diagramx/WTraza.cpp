...
BOOL CVarTraza::PrepareControl(CWnd *pariente,CRect &area,CFont *font)
{
    ...
    if (!Create( WS_CLIPSIBLINGS | WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_SHOWSELALWAYS | LVS_SINGLESEL|WS_HSCROLL|WS_VSCROLL, area , pariente , ID_TRAZAVAR ))
	   return FALSE;
	...
	lvcolumn.iSubItem = 1;
	lvcolumn.iOrder = 1;
	lvcolumn.cx = rect.Width() - 100 - 2;
	InsertColumn(1, &lvcolumn);

    DeleteAllItems( );

	InsertaUno(0,"","");
	InsertaUno(1,"FSalida","");
	InsertaUno(2,"FEntrada","");
	InsertaUno(3,"Campo","");
	InsertaUno(4,"PARAMETRO","");
	InsertaUno(5,"","");
...
m_rect.left = theApp.GetProfileInt("TRAZA","PLEFT",6);
m_rect.right = theApp.GetProfileInt("TRAZA","PRIGHT",455);
m_rect.top = theApp.GetProfileInt("TRAZA","PTOP",6);
m_rect.bottom = theApp.GetProfileInt("TRAZA","PBOTTOM",555);

CreateEx(0, AfxRegisterWndClass(NULL),
	"Traza Interna 1.1", WS_POPUPWINDOW  | WS_CAPTION | WS_MAXIMIZEBOX | WS_MINIMIZEBOX | WS_THICKFRAME | WS_CLIPCHILDREN,
...
if (!m_visible)
   ...
   */
   ...
   r.bottom -= 200;
   int bottom;
   bottom = theApp.GetProfileInt("TRAZA","SBOTTOM",r.bottom);
   r.bottom = bottom;
   ...
   int nvar = theApp.GetProfileInt("TRAZA","NVAR",0);
   if (nvar > 0)
   {
	   ...
	   for (int i = 0;i < nvar;i++)
	   {
			CString s;
			s.Format("VAR_%d",i);		
			s = theApp.GetProfileString("TRAZA",s,m_vars.GetItemText( i, 0));
			m_vars.InsertaUno(i,s,"");
	   }
   }
...

{
	...
	MSG msg;
	while (PeekMessage((LPMSG)&msg,NULL,0,0,PM_NOREMOVE) && ::GetMessage((LPMSG)&msg,NULL,0,0))
	{
...
if (m_edit.m_hWnd)
  {
     ...
     if (!pide_variables && entero == (m_edit.m_linea+1)) 
	 {
		 goto sal_traza;	
	 }
...
licencia_1:	 
	 ...
	 if (!m_modoavance)
		...
		else if (m_modoavance == 3)
		   {		   
		   m_modoavance = 0;
		   p[1] = 0;
		   goto licencia_1;
		   }
		else if (m_modoavance >= 5)
		   {
...
void CWTraza::OnClose() 
{
	...
	theApp.WriteProfileInt("TRAZA","PLEFT",rect.left);
	theApp.WriteProfileInt("TRAZA","PRIGHT",rect.right);
	theApp.WriteProfileInt("TRAZA","PTOP",rect.top);
	theApp.WriteProfileInt("TRAZA","PBOTTOM",rect.bottom);

	m_edit.GetWindowRect(&rect);
	ScreenToClient(&rect);

	/*
	theApp.WriteProfileInt("TRAZA","SLEFT",rect.left);
	theApp.WriteProfileInt("TRAZA","SRIGHT",rect.right);
	theApp.WriteProfileInt("TRAZA","STOP",rect.top);
	*/
	theApp.WriteProfileInt("TRAZA","SBOTTOM",rect.bottom);

	int nvar = m_vars.GetItemCount();
	theApp.WriteProfileInt("TRAZA","NVAR",nvar);
	for (int i = 0;i < nvar;i++)
	{
		CString s;
		s.Format("VAR_%d",i);		
		theApp.WriteProfileString("TRAZA",s,m_vars.GetItemText( i, 0));
	}
...
CTrazaBuscar::CTrazaBuscar(CWnd* pParent /*=NULL*/)
	: CDialog(CTrazaBuscar::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CTrazaBuscar, CDialog)
...
void CFonTraza::OnLButtonDown(UINT nFlags, CPoint point) 
{
	CListCtrl::OnLButtonDown(nFlags, point);
	...
	{		
		...
		Rect.left += (8+GetColumnWidth(0));
		Rect.right = Rect.left + GetColumnWidth(1);
		if (Rect.PtInRect(point))
		{
			int d = point.x - Rect.left;
			CString s,s1;
			CSize t;			
			s = GetItemText(item,1);
			CDC *pdc = GetDC();			
			for (int i = 1;i <= s.GetLength();i++)
			{
				s1 = s.Mid(0,i);
				t = pdc->GetTextExtent( s1 );
				pdc->LPtoDP(&t);
				if (t.cx >= d)
				{					
					if (s.Mid(i-1,1)[0] != ' ')
					{
						int pi = i - 1;
						while(pi > 0 && s.Mid(pi,1)[0] != ' ' && s.Mid(pi,1)[0] != '=' && s.Mid(pi,1)[0] != ';')
							pi--;
						int pf = i - 1;
						while(pf < s.GetLength() && s.Mid(pf,1)[0] != ' ' && s.Mid(pf,1)[0] != '=' && s.Mid(pf,1)[0] != ';')
							pf++;
						s1 = s.Mid(pi,pf-pi);
						s1.TrimRight();
						s1.TrimLeft();
						...
						CString s2;
						s2.Format("%d",atoi(s1));
						if (!s1.IsEmpty() && s1[0] != '|' && s1.Compare(s2))
						{
							CWTraza *pwnd = (CWTraza *)GetParent();
							if (pwnd)
							{								
								pwnd->m_vars.SetItemText(0,0,s1);
								pwnd->m_vars.SetItemText(0,1,"");
								pwnd->PideVar(0);
							}
						}
					}
