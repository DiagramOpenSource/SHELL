...
inline void CDrawButton::Redraw()
{
	if( m_hWnd != NULL )
		Invalidate();
}
...
//	Image functions
BOOL CDrawButton::SetIcon( UINT nID, int nWidth, int nHeight )
{
...
BOOL CDrawButton::SetBitmap( UINT nID, int nWidth, int nHeight )
{
	return SetBitmapCommon( nID, nWidth, nHeight, 0, FALSE /* no mask */ );
}

BOOL CDrawButton::SetMaskedBitmap( UINT nID, int nWidth, int nHeight, COLORREF crTransparentMask )
{
	return SetBitmapCommon( nID, nWidth, nHeight, crTransparentMask, TRUE /* mask */ );
}

BOOL CDrawButton::SetBitmapCommon( UINT nID, int nWidth, int nHeight, COLORREF crTransparentMask, BOOL bUseMask )
{
	// GS980730 added AfxFindResourceHandle to make MFC find resources in DLLs too...
	HINSTANCE hInstResource = AfxFindResourceHandle(MAKEINTRESOURCE(nID),
													RT_BITMAP);
	...
	{
		BITMAP bmpinfo;
		memset((void *)&bmpinfo,0,sizeof(bmpinfo));
		::GetObject(bmTemp,sizeof(bmpinfo),(void *)&bmpinfo);
		nWidth = bmpinfo.bmWidth;/*(bmpinfo.bmWidth * nHeight) / bmpinfo.bmHeight + 1;*/
...
void CDrawButton::SetColorTab(COLORREF crTab)
{	
	m_bColorTab = TRUE;
	m_crColorTab = crTab;
...
BOOL CDrawButton::SetDefaultButton( BOOL bState )
{
	CBASEDLG *pDialog = (CBASEDLG *)GetOwner();
//	ASSERT( pDialog->IsKindOf( RUNTIME_CLASS( CBASEDLG ) ) );
...
int CDrawButton::SetImageOffset( int nPixels )
{
...
int CDrawButton::SetTextOffset( int nPixels ) 
{ 
...
CPoint CDrawButton::SetImagePos( CPoint p ) 
{ 
...
CPoint CDrawButton::SetTextPos( CPoint p ) 
{ 
...
void CDrawButton::CheckPointForCentering( CPoint &p, int nWidth, int nHeight )
{
	CRect	rectControl;
	GetClientRect( rectControl );

	if( p.x == DrawButton_CENTER )	
		p.x = ( ( rectControl.Width() - nWidth ) >> 1 );	
	if( p.y == DrawButton_CENTER )
...
{	
	...
	if( !bDRAWFOCUSONLY )
	{
		...
		if( HasImage() )
		{
			...
			if( m_bUseOffset )
			{
				pt.x = strTitle.IsEmpty() ? DrawButton_CENTER : rectControl.left + m_nImageOffsetFromBorder;
				pt.y = DrawButton_CENTER;
			}
			else
				pt = m_pointImage;
		...
		if ( !strTitle.IsEmpty() )
		{
			...
			if( m_bUseOffset )
			{
				...
				if (m_izquierda)
				{
					pt.x = !HasImage() ? m_nImageOffsetFromBorder : 
						m_nImageWidth + m_nTextOffsetFromImage + m_nImageOffsetFromBorder;
				}
				else
				{
					pt.x = !HasImage() ? DrawButton_CENTER : 
						m_nImageWidth + m_nTextOffsetFromImage + m_nImageOffsetFromBorder +
						(max(0,rcClient.Width()-(m_nImageWidth + m_nTextOffsetFromImage + m_nImageOffsetFromBorder)-sizeText.cx)/2);
				}
						;
//				pt.x = !HasImage() ? DrawButton_CENTER : m_nImageWidth + m_nTextOffsetFromImage + m_nImageOffsetFromBorder;
				pt.y = DrawButton_CENTER; 
			}
			else
				pt = m_pointText;
			...
			int nOffsetFixY = pt.y == DrawButton_CENTER ? -1 : 0;
